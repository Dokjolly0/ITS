    ...
    public class ControllerDipendenti : ControllerBase
    {
        ...
        public String CodiceFiscale(int id)
        {
            ...
            try
            {
                ...
                if (reader.Read())
                {
                    Dipendenti appo = new Dipendenti();
                    appo.Id = Convert.ToInt32(reader["DipendenteID"]); //legge per ogni record l'attributo selezionato, in questo caso voglio l'id dell' articolo
                    appo.Cognome = Convert.ToString(reader["Cognome"]);
                    appo.Nome = Convert.ToString(reader["Nome"]);
                    appo.DataDiNascita = Convert.ToDateTime(reader["DataNascita"]);
                    appo.Sesso = Convert.ToString(reader["Sesso"]);
                    appo.Comune = Convert.ToString(reader["ComuneNascita"]);
                    appo.Provincia = Convert.ToString(reader["ProvinciaNascita"]);
                    appo.Email = Convert.ToString(reader["Email"]);
                    con.Close();
                    con.Open();
                    cmd.Parameters.AddWithValue("@com", appo.Comune);
                    cmd.CommandText = "SELECT * FROM TComuni WHERE Comune = @com"; //torna -1 perché non lo trova
                    reader.Close();
                    ...
                    string codicef = "";
                    bool cnome = false;
                    codicef += CognomeNome(appo.Cognome, cnome);
                    cnome = true;
                    codicef += CognomeNome(appo.Nome, cnome);
                    string appo2 = appo.DataDiNascita.ToString();
                    string data = appo2.Substring(0, 10);
                    codicef += Data(data, appo.Sesso);
                    risp = "Il tuo codice fiscale è: " + codicef + "" + cod;
